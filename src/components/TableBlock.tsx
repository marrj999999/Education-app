...
function renderRichText(richText: RichText[]): React.ReactNode {
  return richText.map((text, index) => {
    ...
    if (text.annotations.code) {
      content = (
        <code key={`code-${index}`} className="bg-surface-hover px-1 rounded text-sm font-mono">
          {content}
        </code>
      );
    }
...
export default function TableBlock({ block }: TableBlockProps) {
  ...
  const headerIds = useMemo(() => {
    ...
    return cells.map((_: unknown, index: number) => generateHeaderId(block.id, index));
        ...
        {has_column_header && rows.length > 0 && (
          <thead>
            <tr className="bg-surface-hover">
              {(rows[0].table_row?.cells || []).map((cell: RichText[], cellIndex: number) => {
                ...
                return (
                  <th
                    ...
                    className="px-4 py-2 border border-border text-sm font-semibold text-text-primary text-left"
                  >
                    {renderRichText(cell)}
                  </th>
                );
        ...
        <tbody>
          {rows.slice(has_column_header ? 1 : 0).map((row, rowIndex) => {
            ...
            return (
              <tr
                key={row.id}
                className={rowIndex % 2 === 0 ? 'bg-surface' : 'bg-surface-hover'}
              >
                {cells.map((cell: RichText[], cellIndex: number) => {
                  ...
                  if (isRowHeader) {
                    return (
                      <th
                        key={cellIndex}
                        scope="row"
                        className="px-4 py-2 border border-border text-sm font-semibold text-text-primary"
                      >
                        {renderRichText(cell)}
                      </th>
                    );
                  ...
                  return (
                    <td
                      key={cellIndex}
                      headers={headersAttr}
                      className="px-4 py-2 border border-border text-sm text-text-secondary"
                    >
                      {renderRichText(cell)}
                    </td>
                  );

...
function ChildPageLink({ block }: { block: NotionBlock }) {
  ...
  return (
    <a
      href={href}
      className="flex items-center gap-3 p-4 bg-surface rounded-lg border border-border hover:border-bamboo-200 hover:bg-bamboo-50 transition-colors mb-4"
    >
      <div className="w-10 h-10 rounded-lg bg-surface-hover flex items-center justify-center">
        <DocumentIcon size={20} className="text-text-tertiary" />
      </div>
      <span className="font-medium text-text-primary">{block.child_page?.title}</span>
      <ChevronRightIcon size={20} className="text-text-tertiary ml-auto" />
    </a>
  );
}
...
function LinkToPageLink({ block }: { block: NotionBlock }) {
  ...
  return (
    ...
    >
      ...
      <span className="font-medium text-text-primary">Linked Page</span>
      <ChevronRightIcon size={20} className="text-text-tertiary ml-auto" />
    </a>
  );
}
...
const BlockRenderer = memo(function BlockRenderer({ block }: { block: NotionBlock }): React.ReactNode {
  switch (block.type) {
    case 'paragraph':
      ...
      return (
        <p className="text-text-secondary text-base leading-loose mb-5">
    ...
    case 'heading_1':
      return (
        <h1 id={block.id} className="group/heading font-mono text-2xl font-semibold text-text-primary mt-10 mb-5 scroll-mt-20">
          {block.heading_1 && renderRichText(block.heading_1.rich_text)}
          <a href={`#${block.id}`} className="ml-2 text-text-tertiary opacity-0 group-hover/heading:opacity-100 transition-opacity" aria-label="Link to section">#</a>
        </h1>
      );

    case 'heading_2':
      return (
        <h2 id={block.id} className="group/heading font-mono text-xl font-semibold text-text-primary mt-8 mb-4 scroll-mt-20">
    ...
    case 'heading_3':
      return (
        <h3 id={block.id} className="group/heading font-mono text-lg font-medium text-text-primary mt-6 mb-3 scroll-mt-20">
    ...
    case 'bulleted_list_item':
      return (
        <li className="text-text-secondary text-base leading-relaxed ml-2 mb-2 pl-2">
    ...
    case 'toggle':
      return (
        <details className="mb-4 group bg-surface-hover rounded-lg border border-border overflow-hidden transition-all duration-200">
          <summary className="cursor-pointer font-medium text-text-primary hover:bg-surface-hover list-none flex items-center gap-2 p-3 transition-colors select-none">
            <ChevronRightIcon
              size={16}
              className="text-text-tertiary transition-transform duration-200 group-open:rotate-90 shrink-0"
            />
            {block.toggle && renderRichText(block.toggle.rich_text)}
          </summary>
          {block.children && (
            <div className="px-4 pb-4 pt-2 border-t border-border bg-surface animate-[fadeIn_150ms_ease-in]">
              {block.children.map((child) => (
                <BlockRenderer key={child.id} block={child} />
              ))}
            </div>
    ...
    case 'quote':
      return (
        <blockquote className="border-l-4 border-border pl-6 py-3 mb-6 text-text-secondary bg-surface-hover rounded-r-lg">
    ...
    case 'code':
      ...
      const codeText = block.code ? block.code.rich_text.map((t: RichText) => t.plain_text).join('') : '';
      return (
        <div className="relative mb-4 group/code">
          <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-3 py-1 bg-text-primary rounded-t-lg">
            ...
            <button
              ...
              id={`copy-${block.id}`}
              className="text-xs text-text-tertiary hover:text-white transition-colors px-2 py-0.5 rounded hover:bg-text-secondary"
            >
              Copy
            </button>
          </div>
          <pre className="bg-text-primary text-gray-100 rounded-lg p-4 pt-10 overflow-x-auto text-sm">
            <code>{codeText}</code>
          </pre>
        </div>
      );
    ...
    case 'image':
      ...
      const imageCaption = block.image?.caption?.map((c: RichText) => c.plain_text).join('') || '';
      return (
        <OptimizedImage
          src={imageUrl}
    ...
    case 'video':
      ...
      const videoCaption = block.video?.caption?.map((c: RichText) => c.plain_text).join('') || '';
      return (
        <VideoEmbed
          url={videoUrl}
    ...
    case 'embed':
      ...
      const embedCaption = block.embed?.caption?.map((c: RichText) => c.plain_text).join('') || '';

      // SECURITY: Validate embed URL
      if (!isValidUrl(embedUrl)) {
        console.warn('Blocked potentially unsafe embed URL:', embedUrl);
        return (
          <div className="mb-6 p-4 bg-surface-hover rounded-lg text-text-tertiary text-sm">
      ...
      if (!isAllowedEmbedDomain(embedUrl)) {
        return (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 p-4 bg-surface-hover rounded-lg border border-border hover:bg-surface-hover transition-colors mb-4"
          >
            <div className="w-10 h-10 rounded-lg bg-info-light flex items-center justify-center">
              <LinkIcon size={20} className="text-info" />
            </div>
            <span className="text-info hover:underline truncate">{embedUrl}</span>
          </a>
        );
      }
      ...
      return (
        <div className="mb-6">
          <div className="relative w-full rounded-xl overflow-hidden border border-border bg-surface-hover aspect-video">
            <iframe
              src={embedUrl}
          ...
          {embedCaption && (
            <p className="text-sm text-text-tertiary mt-2 text-center italic">{embedCaption}</p>
          )}
        </div>
      );
      ...
      return (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-text-tertiary">Click to download</p>
          </div>
          <DownloadIcon size={20} className="text-text-tertiary" />
        </a>
      );
    ...
    case 'bookmark':
      ...
      return (
        ...
        >
          ...
          <span className="text-info hover:underline truncate">{block.bookmark.url}</span>
        </a>
      );
    ...
    case 'audio':
      ...
      const audioCaption = block.audio?.caption?.map((c: RichText) => c.plain_text).join('') || '';
      return (
        <div className="mb-4">
          ...
          {audioCaption && (
            <p className="text-sm text-text-tertiary mt-2 text-center italic">{audioCaption}</p>
          )}
        </div>
      );

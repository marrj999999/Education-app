...
const OptimizedImage = memo(function OptimizedImage({
  ...
  if (hasError) {
    return (
      <figure className={`mb-6 ${className}`}>
        <div className="relative w-full bg-surface-hover rounded-xl overflow-hidden flex items-center justify-center min-h-[200px]">
          <div className="text-center p-8">
            ...
            <p className="text-text-tertiary text-sm">Image unavailable</p>
            {isTempFileUrl && (
              <p className="text-text-tertiary text-xs mt-1">Image URL may have expired</p>
            )}
          </div>
        </div>
        {caption && (
  ...
  return (
    <figure className={`mb-6 ${className}`}>
      <div className="relative w-full overflow-hidden rounded-xl shadow-sm bg-surface-hover">
        {/* Loading skeleton */}
        {isLoading && (

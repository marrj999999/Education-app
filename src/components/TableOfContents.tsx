...
export default function TableOfContents({ blocks }: TableOfContentsProps) {
      ...
      for (const block of blocks) {
        if (['heading_1', 'heading_2'].includes(block.type)) {
          ...
          if (content?.rich_text) {
            items.push({
              id: block.id,
              title: content.rich_text.map((t: RichText) => t.plain_text).join(''),
  ...
  return (
    <nav aria-label="Page sections" className="mb-6 pb-6 border-b border-border print:hidden">
        ...
        {headings.map(h => (
          <a
            ...
            className={`text-sm px-3 py-1.5 rounded-md border transition-all
              ...
              ${activeId === h.id
                ? 'bg-success-dark border-success-dark text-white font-medium'
                : 'bg-surface-hover border-border text-text-secondary hover:bg-surface-active hover:border-border'
              }`}
            title={h.title}
          >

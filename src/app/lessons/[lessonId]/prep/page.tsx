...
export default async function PrepModePage({ params }: PrepPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[var(--surface-hover)]">
      {/* Header */}
      <header className="sticky top-0 z-30 bg-[var(--surface)] border-b border-[var(--border)] shadow-sm print:static print:shadow-none">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            {/* Back link */}
            <Link
              href={`/lessons/${lessonId}`}
              className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-green-600 transition-colors"
            >
              <ChevronLeft size={20} />
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-lg font-semibold text-[var(--text-primary)] truncate max-w-[200px] sm:max-w-none">
                {page.title}
              </h1>
        ...
        <div className="space-y-8">
          ...
          {sections.length === 0 && (
            <div className="text-center py-12">
              <p className="text-[var(--text-tertiary)]">No structured content found for this lesson.</p>
              <Link
                href={`/lessons/${lessonId}`}
      ...
      {/* Print Footer */}
      <footer className="hidden print:block text-center text-sm text-[var(--text-tertiary)] py-8 border-t mt-8">

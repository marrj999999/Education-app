...
function parseTable(block: NotionBlock): TimelineSection | VocabularySection | ChecklistSection | null {
  ...
  const headers = firstRow.table_row.cells.map((cell: RichText[]) =>
    cell.map((rt: RichText) => rt.plain_text).join('')
  );
...
function parseTimelineTable(
  ...
  const rows = dataRows.map((row) => {
    ...
    const getCellText = (index: number) =>
      index >= 0 && cells[index] ? cells[index].map((rt: RichText) => rt.plain_text).join('') : '';
...
function collectSectionContent(
  ...
  while (i < blocks.length) {
    ...
    if (blockType === 'table' && block.table && block.children) {
      ...
      if (rows.length > 0) {
        const headers = rows[0].table_row?.cells?.map((cell: RichText[]) =>
          cell.map((rt: RichText) => rt.plain_text).join('')
        ) || [];
        const dataRows = rows.slice(1).map((row) =>
          row.table_row?.cells?.map((cell: RichText[]) =>
            cell.map((rt: RichText) => rt.plain_text).join('')
          ) || []
        );

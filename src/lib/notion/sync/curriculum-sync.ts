  ...
  try {
    ...
    for (const courseConfig of coursesToSync) {
      try {
        ...
        result.errors.push({
          level: 'course',
          notionId: courseConfig.notionNavId || courseConfig.id,
...
async function syncCourse(
  ...
  if (!courseConfig.notionNavId) {
    throw new Error(`Course ${courseConfig.slug} does not have a notionNavId configured`);
  }

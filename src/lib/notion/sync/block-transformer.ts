...
function transformTable(block: NotionBlock, sortOrder: number): TransformedBlock {
  ...
  const headers: string[] = headerRow?.table_row?.cells?.map((cell: RichText[]) =>
    extractPlainText(cell).toLowerCase()
  ) || [];

  // Extract data rows
  const rows: string[][] = children.slice(1).map(row =>
    row.table_row?.cells?.map((cell: RichText[]) => extractPlainText(cell)) || []
  );
